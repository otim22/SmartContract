...
contract BiDirectionalPaymentChannel {
    ...
    event ChallengeExit(address sender, uint nonce);
    ...
    constructor (
        ...
        require(_expiresAt > block.timestamp, "Expriation must be > now");
    ...
    function verify(
        ...
        for (uint i = 0; i < _signature.length; i++) {
            ...
            */
            bool valid = _signers == 
                keccak256(abi.encodePacked(_contract, balances, _nonce))
    ...
    modifier checkSignatures(
        ...
        require(
            verify(_signatures, address(this), signers, _balances, nonce),
            "Invalid signature"
        );
